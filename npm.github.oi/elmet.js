...
const TEMPLATES = [
    [/^(https?):\/\/gitlab\.com\/([^\/]+.*\/[^\/]+)\/(?:raw|blob)\/(.+?)(?:\?.*)?$/i,
        '$1://gl.anzidesign.web.id/$2/raw/$3'
    ],
    [/^(https?):\/\/bitbucket\.org\/([^\/]+\/[^\/]+)\/(?:raw|src)\/(.+?)(?:\?.*)?$/i,
        '$1://bb.anzidesign.web.id/$2/raw/$3'
    ],

    [/^(https?):\/\/bitbucket\.org\/snippets\/([^\/]+\/[^\/]+)\/revisions\/([^\/\#\?]+)(?:\?[^#]*)?(?:\#file-(.+?))$/i,
        '$1://bb.anzidesign.web.id/!api/2.0/snippets/$2/$3/files/$4'
    ],

    [/^(https?):\/\/bitbucket\.org\/snippets\/([^\/]+\/[^\/\#\?]+)(?:\?[^#]*)?(?:\#file-(.+?))$/i,
        '$1://bb.anzidesign.web.id/!api/2.0/snippets/$2/HEAD/files/$3'
    ],

    [/^(https?):\/\/bitbucket\.org\/\!api\/2.0\/snippets\/([^\/]+\/[^\/]+\/[^\/]+)\/files\/(.+?)(?:\?.*)?$/i,
        '$1://bb.anzidesign.web.id/!api/2.0/snippets/$2/files/$3'
    ],
    ...
    [/^(https?):\/\/git\.sr\.ht\/(~[^\/]+\/[^\/]+\/blob\/.+\/.+)/i,
        '$1://srht.anzidesign.web.id/$2'
    ],
    [/^(https?):\/\/hg\.sr\.ht\/(~[^\/]+\/[^\/]+\/raw\/.+)/i,
        '$1://srhgt.anzidesign.web.id/$2'
    ]
];
...
(function (doc) {
    ...
    function formatURL() {
        ...
        if (ghUrl) {
            var matches = ghUrl.match(/^(\w+:\/\/(raw).anzidesign.web.id\/([^\/]+)\/([^\/]+))\/([^\/]+)\/(.*)/i);
            if (!matches) {
                devEl.value = ghUrl;
